...
pub fn build(b: *std.Build) void {
    ...
    // Use pre-translated BoringSSL headers (translate-c _Pragma workaround for Zig 0.16)
    const ssl_mod = b.addModule("ssl", .{
        .root_source_file = b.path("ssl_translated.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    libdecrepit.root_module.addIncludePath(ssl_source.path("include"));
    libdecrepit.root_module.linkLibrary(libcrypto);
    libdecrepit.root_module.linkLibrary(libssl);
    libdecrepit.root_module.linkSystemLibrary("c", .{});
